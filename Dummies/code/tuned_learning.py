"""
The code tuned_learning.py module performs the same MNIST
classification as the deep_learning.py module. The difference
is that it tunes the neural network to improve accuracy and
performance. It also creates fully connected layers by calling
tf.contrib.layers.fully_connected instead of tf.add and tf.matmul .
"""
...
with tf.contrib.framework.arg_scope([fully_connected],
    ...
    out_layer = fully_connected(
        layer3_drop, out_nodes, activation_fn=None, scope='layer4')
...
with tf.Session() as sess:
    ...
    print('Success rate: ', sess.run(success, feed_dict={
          img_holder: dataset.test.images, lbl_holder: dataset.test.labels,
          train: False}))
