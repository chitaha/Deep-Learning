"""
The code cifar_cnn.py demonstrates how you can use TensorFlow to load
CIFAR-10 images and classify them with a convolutional neural network.
"""
...
for file_index in range(5):
    train_file = open('cifar-10-batches-py/data_batch_' +
                      str(file_index + 1), 'rb')
...
conv1 = tf.layers.conv2d(img_holder, num_filters,
                         filter_size, padding='same', activation=tf.nn.relu)
drop1 = tf.layers.dropout(conv1, keep_prob, training=train)
pool1 = tf.layers.max_pooling2d(drop1, 2, 2)
conv2 = tf.layers.conv2d(pool1, num_filters, filter_size,
                         padding='same', activation=tf.nn.relu)
drop2 = tf.layers.dropout(conv2, keep_prob, training=train)
pool2 = tf.layers.max_pooling2d(drop2, 2, 2)
conv3 = tf.layers.conv2d(pool2, num_filters, filter_size,
                         padding='same', activation=tf.nn.relu)
pool3 = tf.layers.max_pooling2d(conv3, 2, 2)
conv4 = tf.layers.conv2d(pool3, num_filters, filter_size,
                         padding='same', activation=tf.nn.relu)
...
with tf.contrib.framework.arg_scope([tf.contrib.layers.fully_connected],
    ...
    fc2 = tf.contrib.layers.fully_connected(
        fc1, num_categories, activation_fn=None)
...
with tf.Session() as sess:
    ...
    for epoch in range(num_epochs):
        ...
        for batch in range(num_batches):
            ...
            sess.run(optimizer, feed_dict={
                     img_holder: img_batch, lbl_holder: lbl_batch,
                     train: True})
    ...
    print('Accuracy: ', sess.run(accuracy, feed_dict={
          img_holder: test_data, lbl_holder: test_labels, train: False}))
