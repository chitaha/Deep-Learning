"""
The code cluster.py module provides a simple example of a distrib-
uted TensorFlow application. The module doesn’t perform any machine learning,
but demonstrates how a set of worker tasks can combine their efforts to
update a variable.
To be specific, the application creates four workers and uses them to
approximate π.This approximation involves summing together the areas
of the rectangles under the function y = (1 + x 2 ) -1 as x runs from
0 to 1 and multiplying the sum by 4.

The cluster.py module generates four workers to perform the approxima-
tion.
"""
...
spec = tf.train.ClusterSpec({'worker':
                             ['localhost:31415', 'localhost:31416',
                              'localhost:31417', 'localhost:31418']})
...
if not flags.FLAGS.task:
    ...
    server = tf.train.Server(
        spec, job_name='worker', task_index=task_index)
    ...
    with tf.train.MonitoredTrainingSession(master='grpc://localhost:31415',
                                           ...
                                           chief_only_hooks=[output_hook]) as sess:
        sess.run(pi_var)
